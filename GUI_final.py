...
def signIn(frame):
    ...
    e_user.widg.bind('<Return>', lambda event: SignInVer(l_op, e_user.widg.get(), e_pass.widg.get()))
    ...
    e_pass.widg.bind('<Return>', lambda event: SignInVer(l_op, e_user.widg.get(), e_pass.widg.get()))
    ...
    signIn_frame.pack()
    root.bind_all('<Escape>', lambda e: initSignInUp(signIn_frame))
    return
...
def signUp(frame):
    ...
    e_user.widg.bind('<Return>', lambda event: signUpVer(l_op, e_user.widg.get(), e_pass.widg.get(), e_passConf.widg.get()))
    ...
    e_pass.widg.config(show=pass_dot)
    e_pass.widg.bind('<Return>', lambda event: signUpVer(l_op, e_user.widg.get(), e_pass.widg.get(), e_passConf.widg.get()))
    ...
    e_passConf.widg.bind('<Return>', lambda event: signUpVer(l_op, e_user.widg.get(), e_pass.widg.get(), e_passConf.widg.get()))
    ...
    signUp_frame.pack()
    root.bind_all('<Escape>', lambda e: initSignInUp(signUp_frame))
    return
...
def homescreen(frame):
    ...
    root.bind_all('<Escape>', lambda e: initSignInUp(homescreen_frame))
    return
...
def initGG(frame):
    ...
    initGG_frame.pack()
    root.bind_all('<Escape>', lambda e: homescreen(initGG_frame))
    return
...
def guessG(frame, number, choice):
    ...
    guessG_frame.pack()
    root.bind_all('<Escape>', lambda e: initGG(guessG_frame))
    return
...
def changeUser(frame):
    ...
    root.bind_all('<Escape>', lambda e: editProfile(changeUser_frame))
    return
...
def changePass(frame):
    ...
    root.bind_all('<Escape>', lambda e: editProfile(changePass_frame))
    return
...
def delProf(frame):
    ...
    delProf_frame.pack()
    root.bind_all('<Escape>', lambda e: editProfile(delProf_frame))
    return
#Calculator menu
def calcMenu(frame):
    hideFrame(frame)
    ...
    calcMenu_frame.pack()
    root.bind_all('<Escape>', lambda e: homescreen(calcMenu_frame))
    return
...
def normCalc(frame):
    ...
    normCalc_frame.pack()
    root.bind_all('<Escape>', lambda e: calcMenu(normCalc_frame))
    return
...
def baseCalc(frame):
    ...
    baseCalc_frame.pack()
    root.bind_all('<Escape>', lambda e: calcMenu(baseCalc_frame))
    return
...
def quadCalc(frame):
    ...
    e_x2.widg.config(justify='right')
    ...
    e_const.widg.config(justify='right')
    quadCalc_g_dic['column']+= 1
    ...
    quadCalc_frame.pack()
    root.bind_all('<Escape>', lambda e: calcMenu(quadCalc_frame))
    return
...
def specNum(frame):
    ...
    specNum_frame.pack()
    root.bind_all('<Escape>', lambda e: destroySPNLab(l_exp.widg, specNum_frame))
    return
...
def signUpVer(label_obj, user, password, password_conf):
    ...
    if(password==password_conf):
        ...
        label_obj.widg.config(text='Password and confirm password do not match!')
    ...
    if (user==current_user):
        if (password==current_pass):
            if(new_password==new_passwordConfirm):
                ...
                # if new pass doesnt match conf pass
                label_obj.widg.config(text="New password doesnt match confirm password!")
        else:
            # if password is incorrect
            label_obj.widg.config(text="Username or Password is incorrect")
    else:
